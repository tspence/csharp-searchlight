...
namespace Searchlight.Tests
{
    ...
    public class LinqExecutorTests
    {
        ...
        [TestMethod]
        public void MultipleSortCriteria()
        {
            // Try sorting by all fields
            var list = GetTestList();
            var syntax = _src.Parse("", null, "name desc, id asc, hired desc, paycheck asc, onduty desc");
            var result = syntax.QueryCollection(list);
            Assert.AreEqual(result.records.Length, list.Count);
            
            // Now sort by just a date field only
            syntax = _src.Parse("", null, "hired desc");
            result = syntax.QueryCollection(list);
            Assert.IsTrue(result.records[0].hired > result.records[1].hired);
            Assert.IsTrue(result.records[1].hired > result.records[2].hired);
            Assert.IsTrue(result.records[2].hired > result.records[3].hired);
            Assert.IsTrue(result.records[3].hired > result.records[4].hired);
            Assert.IsTrue(result.records[4].hired > result.records[5].hired);
            
            // Now sort by just a date field only
            syntax = _src.Parse(null, null, "hired asc");
            result = syntax.QueryCollection(list);
            Assert.IsTrue(result.records[0].hired < result.records[1].hired);
            Assert.IsTrue(result.records[1].hired < result.records[2].hired);
            Assert.IsTrue(result.records[2].hired < result.records[3].hired);
